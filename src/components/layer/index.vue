<template>
    <div>
        <div class="layer" :class="{'active':visibility,[className]:className,'isFixed':!maskLayer }" :style="{zIndex:zIndex} " v-if="dialog">
            ...
            <div class="layer-position" :style="layerStyle" ref="layer">
                <div class="layer-body" :class="['layer-anim-'+animation]">
                    ...
                    <i class="el-icon-close layer-close" v-if="showClose" @click="_close"></i>
                    <div class="layer-header" v-text="title" :style="{cursor: move?'move':''}" ref="head" v-if="title" @mousedown="_mouseDown"></div>
        ...
        <div class="layui-layer-shade" id="layui-layer-shade5" times="5" style="z-index: 19891018; background-color: rgb(0, 0, 0); opacity: 0.3;"></div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        _completed() {
            ...
            this.dialog = true;
            // 单位为 %时
            if (typeof this.height != 'undefined' && this.height.indexOf("%") != -1) {
                console.log("index.vue设置 高度百分比")
                //如果设置了高，则在内容区添加高度,溢出时显示滚动条
                // 如果窗口高度大于屏幕 高度，即取窗口高度　
                ...
                this.layerHeight = this.getHeight(window) * this.toPoint(this.height)
                this.scrollHeight = this.layerHeight - headHeight - 56 + 'px';
            } else if (typeof this.height != 'undefined') {
                // 单位为 PX
                console.log("index.vue设置 高度px")
                //如果设置了高，则在内容区添加高度,溢出时显示滚动条
                // 如果窗口高度大于屏幕高度，即取窗口高度　
                if (this.getHeight(window) < this.layerHeight) {
                    this.layerHeight = this.getHeight(window);
        ...
        _noScroll() {
            ...
            if (this.getHeight(document) > this.getHeight(window)) {
                document.body.style.paddingRight = "0px";//滚动条大概的宽，防抖动
            }
        },
        _confirm() {
