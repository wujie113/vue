<template>
  <div>
    ...
    <el-dialog :visible.sync="v.formupdate" title="上传提示" :append-to-body="false" :close-on-click-modal="false" :modal="false" :modal-append-to-body="false" width="30%">
      ...
      <el-upload 
:http-request="uploadFile" :action="uploadaction" :show-file-list="false"  accept=".xlsx,.xls" class="upload-demo" :before-upload="beforeUpload" :data="uploaddata" :on-success="handleSuccess" :on-error="handlError">
...
export default {
  ...
  methods: {
     handleCurrentChange(row){
			console.log(row)
			  this.$emit('clickRow', { id: row.id, gtype: 'diba', name: row.name, lng: row.startLng, lat: row.endLat })
		},
    getList() {
