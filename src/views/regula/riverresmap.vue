<template>
  <div class="">
            ...
            ><div>
              <div class="filter-container river">
                <el-input
                  ...
                  v-model="river.listQuery.name"
                  style="width: 200px;"
                  class="filter-item"
                  @keyup.enter.native="handleFilter"
                />
                <el-select
                  v-model="river.listQuery.type"
                  placeholder="请选择类型"
                  clearable
                  class="filter-item"
                  style="width: 130px"
                >
                  <el-option
                    v-for="item in river.typeOptions"
                    :key="item.key"
                    :label="item.label"
                    :value="item.key"
                  />
              ...
              <el-table
                ...
                stripe
                size="mini"
                @current-change="riverhandleCurrentChange"
              >
              ...
              <pagination
                v-show="river.total > 0"
                :total="river.total"
                :page.sync="river.listQuery.pageNo"
                :limit.sync="river.listQuery.pageSize"
                @pagination="rivergetList"
              />

              <el-dialog
                :visible.sync="river.dialogVisibleImg"
                title="图片预览"
          ...
          <keep-alive>
            <div>
              <div class="filter-container lake">
                <el-input
                  placeholder="输入名称搜索"
                  v-model="lake.listQuery.name"
                  style="width: 200px;"
                  class="filter-item"
                  @keyup.enter.native="lakehandleFilter"
                />
                <el-select
                  v-model="lake.listQuery.type"
                  placeholder="请选择类型"
                  clearable
                  style="width: 90px"
                  class="filter-item"
                >
                  <el-option
                    v-for="item in lake.typeOptions"
                    :key="item.key"
                    :label="item.label"
                    :value="item.key"
                  />
                ...
                <el-button
                  ...
                  type="primary"
                  icon="el-icon-search"
                  @click="lakehandleFilter"
                  >查询</el-button
                >
                <el-button type="primary" icon="el-icon-plus" @click="lakeadd"
                  >新增</el-button
                >
              </div>
              <el-table
                v-loading="lake.listLoading"
                :data="lake.list"
                border
                fit
                highlight-current-row
              ...
              <pagination
                v-show="lake.total > 0"
                :total="lake.total"
                :page.sync="lake.listQuery.pageNo"
                :limit.sync="lake.listQuery.pageSize"
                @pagination="lakegetList"
              />
            </div>
          </keep-alive>
        </el-tab-pane>
          ...
          <keep-alive>
            <div>
              <div class="filter-container duty">
                <el-input
                  placeholder="输入责任段名称"
                  v-model="duty.listQuery.name"
                  style="width: 200px;"
                  class="filter-item"
                  @keyup.enter.native="dutyhandleFilter"
                />
                <el-button
                  class="filter-item"
                  type="primary"
                  icon="el-icon-search"
                  @click="dutyhandleFilter"
                  >查询</el-button
                >
                <el-button
                  type="primary"
                ...
                <el-table-column prop="id" label="操作" width="135">
                  <template slot-scope="scope">
                    <el-button
                      ...
                      @click="spot(scope.row);"
                    ></el-button>
                    <el-button
                      @click="dutyedit(scope.row);"
                      type="text"
                      size="mini"
                      icon="el-icon-edit"
                    />
              ...
              <pagination
                v-show="duty.total > 0"
                :total="duty.total"
                :page.sync="duty.listQuery.pageNo"
                :limit.sync="duty.listQuery.pageSize"
                @pagination="dutygetList"
              />
            </div>
          </keep-alive>
        </el-tab-pane>
    ...
    >
      <el-form ref="dutyform" :model="duty.form" label-width="90px">
        ...
        <el-form-item prop="description" label="绘图">
          <el-button type="button" @click="drawRiverLine();"
            >请点击按钮后绘图</el-button
          >
        </el-form-item>
      </el-form>
      <el-upload
        ...
        :auto-upload="false"
        multiple
        :on-preview="dutyhandlePictureCardPreview"
        accept=".jpg,.jpeg,.png,.gif"
        ref="dutyupload"
        :file-list="duty.fileList"
        :before-remove="dutyremovefile"
        :data="duty.uploaddata"
        :on-success="dutyhandleSuccess"
        :on-remove="dutyhandleRemove"
        :on-error="dutyfileerror"
      >
    ...
    >
      <el-form ref="form" :model="river.form" label-width="80px">
        ...
        <el-form-item label="所属水系">
          <el-select
            v-model="river.form.river.id"
            placeholder="请选择水系"
            clearable
            class="filter-item"
          >
            <el-option
              v-for="item in river.sxOptions"
              :key="item.key"
              :label="item.label"
              :value="item.key"
            />
        ...
        <el-form-item prop="description" label="河流描述">
          <el-input
            v-model="river.form.description"
            :rows="4"
            type="textarea"
          />
        </el-form-item>
    ...
    >
      <el-form ref="lakeform" :model="lake.form" label-width="80px">
        ...
        <el-form-item label="所属区域">
          <el-select
            v-model="lake.form.lake.id"
            placeholder="请选择水系"
            clearable
            class="filter-item"
          >
            <el-option
              v-for="item in lake.quOptions"
              :key="item.key"
              :label="item.label"
              :value="item.key"
            />
        ...
        <el-form-item prop="description" label="绘图">
          <el-button type="button" @click="drawlakeAarea();"
            >请点击按钮后绘图</el-button
          >
        </el-form-item>
      </el-form>
      ...
      <el-upload
        ...
        list-type="picture-card"
        :auto-upload="false"
        :on-preview="lakehandlePictureCardPreview"
        accept=".jpg,.jpeg,.png,.gif"
        ref="lakeupload"
        :file-list="lake.fileList"
        :before-remove="lakeremovefile"
        :data="lake.uploaddata"
        :on-success="lakehandleSuccess"
    ...
    >
      <el-table
        v-loading="areauser.userloading"
        :data="areauser.userlist"
    ...
    >
      <div style="width:100%">
        ...
        <el-table style="width:100%" :data="duty.dutyClockList">
          ...
          <el-table-column label="坐标" :formatter="formatterthis">
          </el-table-column>
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button @click="delspot(scope.index, scope.row);"
                >删除</el-button
              >
...
export default {
  ...
  methods: {
    ...
    riversave() {
      ...
      this.river.listLoading = true;
      save(this.river.form)
        .then(response => {
          this.river.uploaddata.bizId = response.data.id;
          console.log(
            "uploadFiles:::::",
            this.$refs.riverupload.uploadFiles.length
          );
          if (
            this.$refs.riverupload.uploadFiles != undefined &&
            this.$refs.riverupload.uploadFiles.length > 0
          ) {
            // this.$refs.riverupload.submit()
            // 上传到服务器
            const imgParams =
              "&bizType=" +
              this.river.uploaddata.bizType +
              "&bizId=" +
              this.river.uploaddata.bizId;
            file(imgParams, this.$refs.riverupload.uploadFiles)
              .then(res => {
    ...
    adduser() {
      ...
      gethzbuserbyareaid({
        areaid: this.commonrow.area.id,
        bizid: this.commonrow.id
      }).then(response => {
    ...
    lakesave() {
      ...
      this.lake.listLoading = true;
      lakessave(this.lake.form)
        .then(response => {
          this.lake.uploaddata.bizId = response.data.id;
          if (
            this.$refs.lakeupload.uploadFiles != undefined &&
            this.$refs.lakeupload.uploadFiles.length > 0
          ) {
            // 上传到服务器
            ...
            const imgParams =
              "&bizType=" +
              this.lake.uploaddata.bizType +
              "&bizId=" +
              this.lake.uploaddata.bizId;
            file(imgParams, this.$refs.lakeupload.uploadFiles)
              .then(res => {
                console.log("file----res", res);
              })
              .catch(errorRes => {});
    ...
    dutysave() {
      ...
      save(this.duty.form)
        .then(response => {
          ...
          this.dialogVisible = true;
          if (
            this.$refs.dutyupload.uploadFiles !== undefined &&
            this.$refs.dutyupload.uploadFiles.length > 0
          ) {
            // this.$refs.dutyupload.submit()
            // 上传到服务器
            const imgParams =
              "&bizType=" +
              this.duty.uploaddata.bizType +
              "&bizId=" +
              this.duty.uploaddata.bizId;
            file(imgParams, this.$refs.dutyupload.uploadFiles)
              .then(res => {
                console.log("file----res", res);
              })
              .catch(errorRes => {});
